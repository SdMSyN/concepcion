...
if (!isset($_SESSION['sessA']))
  ...
  $resGetProds = $con->query($sqlGetProducts);
  $optProducts = '';
  if ($sqlGetProducts && $sqlGetProducts->num_rows > 0) {
    while ($rowGetProducts = $sqlGetProducts->fetch_assoc()) {
      ...
      $optUsers .= '<td>' . $rowGetUsers['categoria'] . '</td>';
      $optUsers .= '<td>' . $rowGetUsers['precio'] . '</td>';
    ...
    $optProducts.='<tr><td colspan="4">No existen usuarios aún.</td></tr>';
  }
