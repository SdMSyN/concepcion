<?php
    ...
    if($_GET['action'] == 'listar'){
        $sqlGetUsers = "SELECT id, CONCAT(nombre,' ',ap,' ',am) as nombre, (SELECT perfil FROM $tPerfil WHERE id=$tUser.perfil_id) as perfil, created, (SELECT nombre FROM $tEst WHERE id=$tUser.activo) as activo FROM $tUser ";
	...
	if($vorder != ''){
        ...
        while ($rowGetUsers = $resGetUsers->fetch_assoc()) {
            ...
            $datos .= '<td>'.$rowGetUsers['activo'].'</td>';
