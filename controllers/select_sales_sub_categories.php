<?php
    ...
    if($resGetSubCategories->num_rows > 0){
        while($rowGetSubCategories = $resGetSubCategories->fetch_assoc()){
            $optSubCategories .= '<div class="col-xs-2 div-img-sales"><img src="./'.$rutaImgSubCat.$rowGetSubCategories['img'].'" class="clickSubCategory img-sales" title="'.$rowGetSubCategories['id'].'" width="10%">'.$rowGetSubCategories['nombre'].'</div>';
        }
    }else{
